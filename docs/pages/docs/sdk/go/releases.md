...
* Added support for restricting the *domains* that processes inside of a Sandbox can connect to, via `OutboundDomainAllowlist` in [`SandboxCreateParams`](/docs/sdk/go/latest/Sandbox#create).
...
* Improved handling of degraded HTTP/2 connections, which addresses intermittent RST\_STREAM errors.
