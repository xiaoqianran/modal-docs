...
您不应直接实例化它，而应使用 \[NewClient]/\[NewClientWithOptions]。
...
* `GRPCStreamInterceptors` (`[]grpc.StreamClientInterceptor`)：GRPCStreamInterceptors 允许自定义 gRPC 流拦截器以进行遥测、跟踪和可观察性。这些附加在 Modal 的内置流拦截器（标头注入）之后。请注意，Modal gRPC API 不被视为公共 API，并且可以在没有警告的情况下进行更改。
