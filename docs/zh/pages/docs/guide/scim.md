...
1. 创建或选择企业应用程序。
   ...
   * 如果您已经有用于 SSO 的模态企业应用程序，
您可以将其重新用于 SCIM 配置。转到“Entra ID > 企业应用程序”
     并选择应用程序。
   * 否则，请转到“Entra ID > 企业应用程序”并创建应用程序：
     1. 选择“新建应用程序 > 创建您自己的应用程序”。
     2. 输入名称，例如`Modal SCIM`。
...
2. 使用您的 Modal SCIM 凭据配置应用程序。
   ...
   并输入：|入口设置|模态值 |
   | ------------- | ------------------------------------------------ |
   |租户网址 | `https://modal.com/api/<your-workspace>/scim/v2` |
...
1. 创建自定义 SCIM 集成。
   ...
   供应。将其命名为可识别的名称，例如`Modal SCIM`。安
...
2. 配置与您的 Modal SCIM 凭证的集成。
   ...
   |设置|模态值 || ---------------------- | ------------------------------------------------ |
   | SCIM 版本 | 2.0 |
   | SCIM 基本 URL | `https://modal.com/api/<your-workspace>/scim/v2` |
   |授权方式 |不记名令牌 |
...
|分页|是的 |                               |
|创建、更新和删除用户 |是的 |                               |
|创建、更新和删除组 |是的 |                               ||使用 PATCH | 更新组成员资格是的 |                               |
